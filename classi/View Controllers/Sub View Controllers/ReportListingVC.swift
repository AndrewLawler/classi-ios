...
class ReportListingVC: UIViewController {
    ...
    @objc func reported() {
        if userID != nil {
            self.presentClassiAlertOnMainThread(title: "Reported Listing", message: "You have reported this listing of a \(listing.car.make) \(listing.car.model)", buttonTitle: "Ok")
        }
        else {
